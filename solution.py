...
==============================================================
Student ID: AI20K-2A202600698
Name: Nguyen Quang Khanh An
...
def validate(data):
    ...
    for record in data:
        ...
        if price_val <= 0 or not category or str(category).strip() == '':
            error_count += 1
        else:
            valid_records.append(record)

    print(f"Validation complete. Kept: {len(valid_records)} valid records, Dropped: {error_count} invalid records")
...
def transform(data):
    ...
    """
    if not data:
        df = pd.DataFrame(columns=['id', 'product', 'price', 'category', 'discounted_price', 'processed_at'])
        return df
